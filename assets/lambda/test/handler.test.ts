...
describe('handler', () => {
  ...
  test('should throw error when addr is missing', async () => {
    ...
    await expect(handler(event, mockContext, mockCallback)).rejects.toThrow('addr and imageUri are required.');
  });
  ...
  test('should send SNS notification when vulnsTopicArn is provided', async () => {
    ...
    await expect(handler(event, mockContext, mockCallback)).rejects.toThrow('vulnerabilities or end-of-life (EOL) image detected');
